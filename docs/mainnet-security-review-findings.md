...
| **Full Sepolia launch rehearsal** | ✅ **PASSED 2026-06-30** — fresh deploy → tokens/pools → mint posture → governance handoff → both verify gates (`verify-governance` **55/55**, `verify-posture-post` green). Validated the launch sequence end-to-end incl. G-1/G-2. Surfaced + fixed 5 launch-process gaps (none contract bugs). Record: `docs/sepolia-rehearsal-2026-06-30.md`; fixes: PR #207. |
...
- **Status:** ✅ MITIGATED (escape hatch implemented 2026-06-30):
  ...
  - **DECISION (2026-06-30, @timogilvie): FundingVault stays OFF the initial mainnet launch path.** It is not deployed by `deploy-mainnet.js`; the three launch tokens (HMESS/HLEAD/HROUT) launch with pools directly, so the gated pre-pool path isn't exercised at go-live. `governance-policy.json` keeps `optional: true`. The `cancelGraduation` escape hatch ships anyway as forward-looking hardening for when FundingVault IS brought on. **Bringing it on later requires the M-10 fix below** (decided: Option A — governance pre-creates the pool).
...
- **M-3 — Reward "cap" recomputed live from mutable param.** [HokusaiToken.sol:154-163,269-275](contracts/HokusaiToken.sol#L154-L163). `rewardCap = REWARD_CAP_MULTIPLIER * tokensPerDeltaOne()`; raising the param (GOV, no delay) unlocks more headroom → reward supply is governance-bounded, not a hard cap. **Tracked: HOK-2409** — decouple the cap from the reward rate, make it increase-only + timelock-gated via TokenManager (immutable ceiling rejected: blocks rewarding a genuinely-improving model). Pre-mainnet decision: do it now (cheap) or risk-accept the Safe-controlled cap.
...
- **M-10 — Factory `createPool*` (`onlyOwner`) vs `FundingVault.graduate()`.** [HokusaiAMMFactory.sol:142-267](contracts/HokusaiAMMFactory.sol#L142-L267) vs [FundingVault.sol:371](contracts/FundingVault.sol#L371). Post-handoff factory owner = timelock, so `graduate()`'s `createPool` call reverts; it ALSO calls `tokenManager.grantRole(MINTER_ROLE, pool)`, which needs TM admin → also reverts for a hot GRADUATOR key. Making FundingVault the owner instead would brick factory admin/pause. **DEFERRED with FundingVault (H-5 decision: off the initial mainnet path).** **Decided approach when revisited (2026-06-30): Option A — governance (timelock/Safe) pre-creates and minter-authorizes the pool, and `graduate()` is refactored to `graduate(modelId, pool, minTokensOut)` doing only the USDC-seeding buy + distribution, so FundingVault holds no pool-creation or minting power.** (Rejected: granting FundingVault `POOL_CREATOR_ROLE`+minter-admin — re-introduces an H-1-class arbitrary-minter risk.)
...
| # | Action | Owner | Done |
|---|--------|-------|------|
| G-1 | Run + verify governance handoff (`verify-governance.js` green for all policy contracts; deployer revoked) | | ◐ **Process validated end-to-end on Sepolia 2026-06-30** (`verify-governance` 55/55, deployer revoked, timelock owns structural contracts — see `docs/sepolia-rehearsal-2026-06-30.md`); ☐ run on mainnet at launch |
| G-2 | Call + assert `disableLegacyMints()` on mainnet before trading | | ◐ **Rehearsed on Sepolia 2026-06-30** (`disableLegacyMints` applied + asserted green via posture verify); ☐ run on mainnet |
...
| H-5 | Confirm FundingVault launch-path; if used, add cancel/refund recovery | @timogilvie | ✅ DECIDED — FundingVault OFF initial mainnet path (not deployed); `cancelGraduation` ships as forward-looking hardening; M-10 fix (Option A) deferred with it |
...
| M-6 | Reconcile launch allocations vs lock doc + re-sign; fill all placeholder addresses | @timogilvie | ◐ Placeholders filled (supplier `0xD1Eb…8136`); final mainnet allocations applied + locked (HMESS 2.5M/10M, HLEAD 1.25M/10M, HROUT 2.5M/10M) in `locked-economics.json` with the drift-guard extended to allocations; ☐ **counter-sign the mainnet block in `mainnet-launch-token-parameter-lock.md`** |
| — | Run `npm run echidna:all` (+ fuzz-long dispatch) on frozen commit; record results | | ☐ |
| — | Sepolia delta check from frozen commit (`verify:launch-posture:sepolia`) | | ◐ **Full Sepolia rehearsal PASSED 2026-06-30** (fresh deploy → handoff → both gates green; `docs/sepolia-rehearsal-2026-06-30.md`); ☐ re-run from the frozen commit |
