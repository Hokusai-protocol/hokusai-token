...
- [ ] Launch-phase cumulative mint cap configured + a `maxReward` per-mint backstop; conservative vs. the `tokensPerDeltaOne = 250,000` economics (Decision 6).
...
- [`mainnet-launch-token-parameter-lock.md`](mainnet-launch-token-parameter-lock.md): **RESOLVED (HOK-2199)** — `tokensPerDeltaOne` is 250,000 (owner-confirmed 2026-06-09), `maxReward` 2.5M, starting per-model budget 1.5M, vesting 10% immediate / 90% over 1 year. These are pinned in `scripts/configs/locked-economics.json` and enforced by `test/scripts/launchEconomicsConsistency.test.js`; the parameter-lock doc and live configs are reconciled to those values.
