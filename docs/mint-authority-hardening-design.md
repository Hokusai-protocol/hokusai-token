...
5. **Launch cap + fraud watch (deterministic bound + detection).** *(Revised per review — Decision 6.)* Define: the **launch-phase cumulative cap** (per-model per-epoch and/or a global mint budget) and how governance raises it with operating history; the per-mint `maxReward` backstop value (against `tokensPerDeltaOne = 500,000`); concrete **monitoring thresholds** on `DeltaOneAccepted` (unexpected recipients, statistically implausible DeltaOne jumps, velocity); and — critically — a **pause authority that can act faster than the mint drain rate** ("monitoring + pause" is only a control if someone/something can trip it in time). The deterministic cap exists precisely because human response may be slower than drain.
...
## 9. Clarifying questions (owner input needed)

These gate the implementation spec and the launch plan:

1. **Will any legacy `DeltaVerifier` mint entrypoint remain callable on mainnet?** (Recommend: no — disable them, or put attester verification on all of them. Decision 9.)
2. **Is the launch attester an EOA hardware wallet, a Safe, or another contract wallet?** Determines EOA `ecrecover` vs. **EIP-1271** (§7 Q2).
3. **What is the intended mainnet per-mint `maxReward`, and the maximum acceptable token loss before pause?** Sets the launch cap (Decision 6, §7 Q5).
4. **Are deterministic model-weight commitments available today, or is that pipeline work before launch?** Gates whether lineage (Decision 8) can ship at launch.
...
- [`mainnet-launch-token-parameter-lock.md`](mainnet-launch-token-parameter-lock.md): specify the planned `maxReward` per mint and the cumulative launch cap against the locked `tokensPerDeltaOne = 500,000` economics.
