...
8. **We will record a verifiable model-weight lineage chain.** Each accepted mint commits to **both** the `baseline` and `candidate` weight commitments (signed + emitted), and the contract **links** them: it stores the canonical head per model (`mapping(modelId ⇒ bytes32 head)`), requires `payload.baselineCommitment == head[modelId]`, then advances `head[modelId] = payload.candidateCommitment`. This makes the mint history a hash-linked chain — like a blockchain — that anyone can follow/rebuild from the genesis (initial model deploy) to today's head, each link an attested DeltaOne improvement. Resolves candidate-vs-baseline (= **both, linked**) and strengthens security: a forged mint can't invent an arbitrary baseline (it must parent off the on-chain head). See §4 Axis A — "Model-weight lineage chain".
