...
3. **Reward bound → layered control (see Decision 6 / §7 Q5).** A single hard `maxReward` would risk clipping a legitimate breakthrough (esp. the **router**, where current results are weak so a large early jump is both likely and desirable). Recommended layering instead:
   ...
   - **Recommendation (per-model budget, kept small):** `mintBudgetRemaining[modelId]`, decremented per mint, **topped up by the Safe (separate from the attester** — a compromised attester must not refill its own budget). A Safe top-up is a straightforward multisig tx (minutes-to-hours), so keep the standing budget small — e.g. a starting **1–2M** for HROUT (a few legitimate mints) — and refill as genuine improvement is earned; it grows toward 14.5M only as the model actually improves. A legit fast-improver is paid in full per mint and at worst briefly deferred, never forfeited. *(Optional v2 ergonomics: an auto-replenishing "drip" — budget accrues X/epoch up to a cap — removes manual top-ups and bounds drain rate automatically; not launch surface.)*
   - **`maxReward` per mint:** size to the largest *single-eval* jump you'd plausibly approve (not the lifetime total). **Input needed:** the biggest single improvement one contribution could realistically make early on (e.g. ~10 points ⇒ `maxReward ≈ 2.5M`). The human attester is the real per-mint check; `maxReward` only blocks the impossible.
   - **Reconcile `tokensPerDeltaOne`:** parameter-lock records `500,000` for models 27/28/30; you cited **250,000** for HROUT — pin the final mainnet value (§10).
