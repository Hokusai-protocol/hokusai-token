...
describe("DeltaVerifier Vesting Integration (DeployableTokenManager)", function () {
  let deltaVerifier;
  let tokenManager;
  ...
  it("mints only the liquid slice to the contributor via submitEvaluation", async function () {
  ...
  it("submitEvaluationWithMultipleContributors does not revert on DeployableTokenManager", async function () {
    const contributors = [
    ...
    await expect(
      deltaVerifier.submitEvaluationWithMultipleContributors(MODEL_ID_UINT, evaluationData, contributors)
    ).to.not.be.reverted;
