...
Still open (see §7): EIP-712 field list specifics; what exactly the model commitment covers (candidate-only vs. candidate+baseline; content hash vs. Merkle root); attester-set/threshold mechanics; the human-signing UX; the long-term decentralized-attestation + **privacy** design (foundations now, full solution later); and migration/cutover.
...
1. **Exact EIP-712 field list, incl. the model commitment.** Confirm the struct fields (modelId, anchors, scores/costs, contributors, idempotency key; domain = chainId + verifying contract) and pin the **model/weights commitment**: (a) candidate-only vs. candidate **+ baseline** (the delta has two endpoints; the pipeline's attestation already hashes both `baseline_model_id` and `new_model_id`, so committing to both weight artifacts gives a verifiable lineage); (b) a single content hash vs. a **Merkle root** over weight shards (the latter is friendlier to partial/streaming and ZK verification later); (c) hash function/format (match the existing `0x`+64-hex sha256 anchor convention); (d) a **reproducible, deterministic weights serialization** the pipeline can commit to at eval time (a dependency to confirm pipeline-side). The contract treats it as opaque `bytes32` regardless.
