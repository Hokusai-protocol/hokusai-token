...
describe("Phase 6: Governance & Safety", function () {
    ...
    describe("Parameter Adjustment", function () {
        ...
        it("Should allow setting CRR to 60%", async function () {
            await hokusaiAMM.setParameters(600000, TRADE_FEE);
            expect(await hokusaiAMM.crr()).to.equal(600000);
        });

        it("Should allow setting CRR to 80%", async function () {
            await hokusaiAMM.setParameters(800000, TRADE_FEE);
            expect(await hokusaiAMM.crr()).to.equal(800000);
        });
