...
describe('EventPublisher', () => {
  ...
  describe('publish', () => {
    ...
    test('should validate message before publishing', async () => {
      ...
      await expect(publisher.publish(invalidMessage as any)).rejects.toThrow();
    ...
    test('should handle Redis errors', async () => {
      ...
      await expect(publisher.publish(validTokenDeployedMessage)).rejects.toThrow(
  ...
  describe('publishBatch', () => {
    test('should publish multiple messages atomically', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/unbound-method -- asserting on a jest mock fn reference, not invoking it
    ...
    test('should rollback on batch failure', async () => {
      ...
      await expect(publisher.publishBatch(messages)).rejects.toThrow();
  ...
  describe('publishWithRetry', () => {
    test('should retry on temporary failures', async () => {
      mockRedis.lPush.mockRejectedValueOnce(new Error('Temporary error')).mockResolvedValueOnce(1);
    ...
    test('should fail after max retries', async () => {
      ...
      await expect(publisher.publishWithRetry(validTokenDeployedMessage, 3, 10)).rejects.toThrow(
        'Persistent error',
      );
  ...
  describe('confirmDelivery', () => {
    test('should confirm message exists in queue', async () => {
      mockRedis.lRange.mockResolvedValue([JSON.stringify(validTokenDeployedMessage)]);

      const confirmed = await publisher.confirmDelivery(validTokenDeployedMessage.model_id);
    ...
    test('should handle large queues efficiently', async () => {
      ...
      const messages = Array(1000)
        .fill(null)
        .map((_, i) => JSON.stringify({ ...validTokenDeployedMessage, model_id: `model_${i}` }));
