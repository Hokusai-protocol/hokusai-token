/**
 * Test Alert Scenarios
 *
 * Run different test alerts to verify the monitoring system
 */

import { SESClient, SendEmailCommand } from '@aws-sdk/client-ses';
import * as dotenv from 'dotenv';

dotenv.config();

const emailFrom = process.env.ALERT_EMAIL_FROM || 'tim@hokus.ai';
const emailTo = process.env.ALERT_EMAIL_TO || 'me@timogilvie.com';
const awsRegion = process.env.AWS_REGION || 'us-east-1';

const sesClient = new SESClient({ region: awsRegion });

// Alert scenarios with blockchain references
const scenarios = {
  'reserve-drop': {
    title: '⚠️ Reserve Drop Alert',
    priority: 'HIGH' as const,
    description: 'Pool reserve dropped below threshold',
    poolAddress: '0x58565F787C49F09C7Bf33990e7C5B7208580901a',
    txHash: '0x1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b',
    blockNumber: 7234567,
    details: {
      pool: 'model-conservative-001',
      poolContract: '0x58565F787C49F09C7Bf33990e7C5B7208580901a',
      currentReserve: '$8,500 USDC',
      previousReserve: '$10,000 USDC',
      dropPercentage: '15%',
      threshold: '10%'
    }
  },
  'large-trade': {
    title: '📊 Large Trade Detected',
    priority: 'MEDIUM' as const,
    description: 'Unusually large trade detected',
    poolAddress: '0xEf815E7F11eD0B88cE33Dd30FC9568f7F66abC5a',
    txHash: '0x9f8e7d6c5b4a3918273645b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5',
    blockNumber: 7234589,
    details: {
      pool: 'model-aggressive-002',
      poolContract: '0xEf815E7F11eD0B88cE33Dd30FC9568f7F66abC5a',
      tradeType: 'BUY',
      tradeSize: '$15,000 USDC',
      buyer: '0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb1',
      priceImpact: '3.2%'
    }
  },
  'pool-paused': {
    title: '🛑 Pool Paused',
    priority: 'CRITICAL' as const,
    description: 'AMM pool has been paused',
    poolAddress: '0x76A59583430243D595E8985cA089a00Cc18B73af',
    txHash: '0x3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d',
    blockNumber: 7234601,
    details: {
      pool: 'model-balanced-003',
      poolContract: '0x76A59583430243D595E8985cA089a00Cc18B73af',
      pausedBy: '0x3018Cf81729c932Bc3E733A264e5F4a0A08deD5B',
      timestamp: new Date().toISOString(),
      reason: 'Manual pause by owner'
    }
  },
  'high-volume': {
    title: '📈 High Trading Volume',
    priority: 'LOW' as const,
    description: 'Unusual trading volume detected',
    poolAddress: '0x58565F787C49F09C7Bf33990e7C5B7208580901a',
    txHash: '0x5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f',
    blockNumber: 7234623,
    details: {
      pool: 'model-conservative-001',
      poolContract: '0x58565F787C49F09C7Bf33990e7C5B7208580901a',
      trades24h: '156',
      volume24h: '$45,000 USDC',
      averageTradeSize: '$288 USDC',
      threshold: '100 trades/day'
    }
  },
  'price-slippage': {
    title: '💱 High Price Slippage',
    priority: 'HIGH' as const,
    description: 'Price slippage exceeded threshold',
    poolAddress: '0xEf815E7F11eD0B88cE33Dd30FC9568f7F66abC5a',
    txHash: '0x7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b',
    blockNumber: 7234645,
    details: {
      pool: 'model-aggressive-002',
      poolContract: '0xEf815E7F11eD0B88cE33Dd30FC9568f7F66abC5a',
      previousPrice: '$0.0120',
      currentPrice: '$0.0105',
      slippage: '12.5%',
      threshold: '5%',
      timeWindow: '5 minutes'
    }
  }
};

type ScenarioKey = keyof typeof scenarios;

async function sendAlert(scenarioKey: string): Promise<boolean> {
  if (!(scenarioKey in scenarios)) {
    console.error(`Unknown scenario: ${scenarioKey}`);
    console.log(`Available scenarios: ${Object.keys(scenarios).join(', ')}`);
    return false;
  }

  const scenario = scenarios[scenarioKey as ScenarioKey];
  const subject = `${scenario.title} - Hokusai AMM Monitoring [${scenario.priority}]`;

  // Generate Etherscan links
  const explorerBase = 'https://sepolia.etherscan.io';
  const txLink = `${explorerBase}/tx/${scenario.txHash}`;
  const blockLink = `${explorerBase}/block/${scenario.blockNumber}`;
  const poolLink = `${explorerBase}/address/${scenario.poolAddress}`;

  const htmlBody = `
<!DOCTYPE html>
<html>
<head>
  <style>
    body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
    .container { max-width: 600px; margin: 0 auto; padding: 20px; }
    .header { background: ${getPriorityColor(scenario.priority)}; color: white; padding: 20px; border-radius: 8px 8px 0 0; }
    .content { background: #f7fafc; padding: 20px; border: 1px solid #e2e8f0; }
    .alert-box { background: #fff3cd; border-left: 4px solid #ffc107; padding: 15px; margin: 15px 0; }
    .details { background: white; padding: 15px; margin: 15px 0; border-radius: 4px; }
    .blockchain-ref { background: #e6f3ff; border-left: 4px solid #2196F3; padding: 15px; margin: 15px 0; }
    .etherscan-link { display: inline-block; background: #21325b; color: white; padding: 8px 16px; text-decoration: none; border-radius: 4px; margin: 5px 5px 5px 0; }
    .etherscan-link:hover { background: #2f4a7c; }
    .footer { background: #2d3748; color: #a0aec0; padding: 15px; text-align: center; border-radius: 0 0 8px 8px; }
    .mono { font-family: 'Courier New', monospace; font-size: 0.9em; }
  </style>
</head>
<body>
  <div class="container">
    <div class="header">
      <h1 style="margin: 0;">${scenario.title}</h1>
      <p style="margin: 5px 0 0 0; opacity: 0.9;">Priority: ${scenario.priority}</p>
    </div>
    <div class="content">
      <div class="alert-box">
        <strong>⚠️ Alert Description</strong>
        <p>${scenario.description}</p>
      </div>

      <div class="blockchain-ref">
        <h3 style="margin-top: 0;">🔗 Blockchain Reference</h3>
        <p><strong>Transaction Hash:</strong><br/><span class="mono">${scenario.txHash}</span></p>
        <p><strong>Block Number:</strong> ${scenario.blockNumber.toLocaleString()}</p>
        <p><strong>Pool Contract:</strong><br/><span class="mono">${scenario.poolAddress}</span></p>
        <div style="margin-top: 15px;">
          <a href="${txLink}" class="etherscan-link">📄 View Transaction</a>
          <a href="${blockLink}" class="etherscan-link">🔲 View Block</a>
          <a href="${poolLink}" class="etherscan-link">📝 View Contract</a>
        </div>
      </div>

      <h3>Details</h3>
      <div class="details">
        ${Object.entries(scenario.details).map(([key, value]) => `
          <p><strong>${formatKey(key)}:</strong> ${value}</p>
        `).join('')}
      </div>
      <p><strong>Timestamp:</strong> ${new Date().toISOString()}</p>
    </div>
    <div class="footer">
      <p style="margin: 5px 0;">Hokusai AMM Monitoring - Testnet (Sepolia)</p>
    </div>
  </div>
</body>
</html>
  `;

  const textBody = `
${scenario.title}
Priority: ${scenario.priority}

${scenario.description}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔗 BLOCKCHAIN REFERENCE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Transaction Hash: ${scenario.txHash}
Block Number: ${scenario.blockNumber.toLocaleString()}
Pool Contract: ${scenario.poolAddress}

Etherscan Links (Sepolia):
📄 Transaction: ${txLink}
🔲 Block: ${blockLink}
📝 Contract: ${poolLink}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DETAILS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

${Object.entries(scenario.details).map(([key, value]) => `${formatKey(key)}: ${value}`).join('\n')}

Timestamp: ${new Date().toISOString()}

---
Hokusai AMM Monitoring - Testnet (Sepolia)
  `;

  try {
    console.log(`\n📧 Sending ${scenarioKey} alert...`);

    const command = new SendEmailCommand({
      Source: emailFrom,
      Destination: { ToAddresses: [emailTo] },
      Message: {
        Subject: { Data: subject, Charset: 'UTF-8' },
        Body: {
          Html: { Data: htmlBody, Charset: 'UTF-8' },
          Text: { Data: textBody, Charset: 'UTF-8' }
        }
      }
    });

    const response = await sesClient.send(command);
    console.log(`✅ Alert sent! Message ID: ${response.MessageId}`);
    return true;
  } catch (error) {
    console.error(`❌ Failed to send alert:`, (error as Error).message);
    return false;
  }
}

function getPriorityColor(priority: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW'): string {
  const colors = {
    'CRITICAL': 'linear-gradient(135deg, #dc2626 0%, #991b1b 100%)',
    'HIGH': 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)',
    'MEDIUM': 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
    'LOW': 'linear-gradient(135deg, #10b981 0%, #059669 100%)'
  };
  return colors[priority] || colors['MEDIUM'];
}

function formatKey(key: string): string {
  return key
    .replace(/([A-Z])/g, ' $1')
    .replace(/^./, str => str.toUpperCase())
    .replace(/24h/, '24h')
    .trim();
}

// CLI handling
async function main(): Promise<void> {
  const args = process.argv.slice(2);

  if (args.length === 0 || args[0] === '--help') {
    console.log('\n🧪 Hokusai Alert Test Script\n');
    console.log('Usage: npx tsx src/test-alerts.ts [scenario] [scenario2...]\n');
    console.log('Available scenarios:');
    Object.keys(scenarios).forEach(key => {
      const scenario = scenarios[key as ScenarioKey];
      console.log(`  - ${key}: ${scenario.description}`);
    });
    console.log('\nExamples:');
    console.log('  npx tsx src/test-alerts.ts reserve-drop');
    console.log('  npx tsx src/test-alerts.ts large-trade high-volume');
    console.log('  npx tsx src/test-alerts.ts all\n');
    return;
  }

  const toTest = args[0] === 'all' ? Object.keys(scenarios) : args;

  console.log(`\n📬 Sending to: ${emailTo}`);
  console.log(`📤 From: ${emailFrom}\n`);

  let sent = 0;
  let failed = 0;

  for (const scenario of toTest) {
    const success = await sendAlert(scenario);
    if (success) sent++;
    else failed++;

    // Small delay between emails
    if (toTest.length > 1) {
      await new Promise(resolve => setTimeout(resolve, 2000));
    }
  }

  console.log(`\n✅ Sent: ${sent}`);
  if (failed > 0) console.log(`❌ Failed: ${failed}`);
  console.log(`\n📬 Check your inbox at ${emailTo}\n`);
}

main().catch(console.error);
