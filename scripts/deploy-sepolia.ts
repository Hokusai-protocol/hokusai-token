...
class SepoliaDeployer {
  ...
  async deployContract(name: string, args: any[] = []): Promise<string> {
    ...
    // Deploy contract with gas limit buffer
    ...
    await this.verifyContract(address, name, args);
    
    return address;
  }

  async verifyContract(address: string, name: string, args: any[]): Promise<void> {
    try {
      ...
      console.log(`⚠️  Failed to verify ${name} on Etherscan:`, error);
    }
  }
  ...
  async deployAllContracts(): Promise<ContractAddresses> {
    ...
    this.addresses.modelRegistry = await this.deployContract('ModelRegistry');
...
async function main() {
  try {
    ...
    // Deploy contracts
    const addresses = await deployer.deployAllContracts();
