...
export class SSMParameterStore {
  ...
  async getParameter(name: string, isRequired = true): Promise<string | undefined> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (isLastAttempt) {
          logger.error(
            `Failed to retrieve SSM parameter ${fullPath} after ${maxAttempts} attempts:`,
            error,
          );
        ...
        logger.warn(
          `Failed to retrieve SSM parameter ${fullPath} (attempt ${attempt}/${maxAttempts}), retrying in ${totalDelay.toFixed(0)}ms:`,
          error.message,
        );
  ...
  async getParameters(names: string[]): Promise<Record<string, string | undefined>> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        logger.debug(
          `Fetching ${fullPaths.length} SSM parameters (attempt ${attempt}/${maxAttempts})`,
        );
        ...
        logger.warn(
          `Failed to retrieve SSM parameters (attempt ${attempt}/${maxAttempts}), retrying in ${totalDelay.toFixed(0)}ms:`,
          error.message,
        );
        await new Promise((resolve) => setTimeout(resolve, totalDelay));
      }
