...
describe("HokusaiToken", function () {
  ...
  beforeEach(async function () {
    [owner, controller, governor, user1, user2, ...addrs] = await ethers.getSigners();
  ...
  describe("Constructor", function () {
    it("Should deploy with custom name, symbol, controller, params, and initial supply", async function () {
      const customName = "Custom Model Token";
      const customSymbol = "CMT";
      const initialSupply = parseEther("1000");

      token = await Token.deploy(customName, customSymbol, controller.address, await params.getAddress(), initialSupply, 0, 0, 0, ZeroAddress);
      ...
      expect(await token.params()).to.equal(await params.getAddress());
    ...
    it("Should emit ControllerUpdated event during deployment", async function () {
      const tx = await Token.deploy("Test Token", "TEST", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
    ...
    it("Should set the deployer as owner", async function () {
      token = await Token.deploy("Test Token", "TEST", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    ...
    it("Should have correct initial supply", async function () {
      ...
      token = await Token.deploy("Test Token", "TEST", controller.address, await params.getAddress(), initialSupply, 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    ...
    it("Should revert with empty name", async function () {
      await expect(
        Token.deploy("", "TEST", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress)
      ).to.be.revertedWith("Token name cannot be empty");
    });

    it("Should revert with empty symbol", async function () {
      await expect(
        Token.deploy("Test Token", "", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress)
      ).to.be.revertedWith("Token symbol cannot be empty");
    });

    it("Should revert with zero address controller", async function () {
      await expect(
        Token.deploy("Test Token", "TEST", ZeroAddress, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress)
      ).to.be.revertedWith("Controller cannot be zero address");
    });

    it("Should revert with zero initial supply", async function () {
      await expect(
        Token.deploy("Test Token", "TEST", controller.address, await params.getAddress(), 0, 0, 0, 0, ZeroAddress)
    ...
    it("Should handle long token names and symbols", async function () {
      ...
      token = await Token.deploy(longName, longSymbol, controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    ...
    it("Should handle unicode characters in name and symbol", async function () {
      ...
      token = await Token.deploy(unicodeName, unicodeSymbol, controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    ...
    it("Should allow same address to be both owner and controller", async function () {
      token = await Token.deploy("Test Token", "TEST", owner.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    ...
    it("Should deploy multiple tokens with different parameters", async function () {
      const token1 = await Token.deploy("Token One", "TOK1", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      const token2 = await Token.deploy("Token Two", "TOK2", user1.address, await params.getAddress(), parseEther("2000"), 0, 0, 0, ZeroAddress);
      const token3 = await Token.deploy("Token Three", "TOK3", user2.address, await params.getAddress(), parseEther("3000"), 0, 0, 0, ZeroAddress);
      ...
      expect(await token1.totalSupply()).to.equal(parseEther("1000"));
      ...
      expect(await token2.totalSupply()).to.equal(parseEther("2000"));
      ...
      expect(await token3.totalSupply()).to.equal(parseEther("3000"));
    });
  });
  ...
  describe("Standard Deployment Tests", function () {
    beforeEach(async function () {
      ...
      token = await Token.deploy("Hokusai Token", "HOKU", controller.address, await params.getAddress(), parseEther("10000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    });

    describe("ERC20 Standard Functionality", function () {
      beforeEach(async function () {
        // Transfer some tokens from controller to user1 for testing
        await token.connect(controller).transfer(user1.address, parseEther("1000"));
      });
    ...
    describe("Burning", function () {
      beforeEach(async function () {
        // Transfer some tokens from controller for testing
        await token.connect(controller).transfer(user1.address, parseEther("1000"));
        await token.connect(controller).transfer(user2.address, parseEther("500"));
      });
    ...
    describe("Integration scenarios", function () {
      it("Should handle complex mint, transfer, and burn flow", async function () {
        ...
        expect(await token.totalSupply()).to.equal(parseEther("10700")); // 10000 initial + 1000 minted - 300 burned (200 + 100)
      });
      ...
      it("Should maintain functionality after controller changes", async function () {
        ...
        expect(await token.totalSupply()).to.equal(parseEther("11200")); // 10000 initial + 1500 minted - 300 burned
      });
    });
  });
  ...
  describe("Custom Token Scenarios", function () {
    it("Should work correctly with AI model specific tokens", async function () {
      ...
      const gptToken = await Token.deploy("GPT Model Token", "GPT", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      const dalleToken = await Token.deploy("DALL-E Model Token", "DALLE", controller.address, await params.getAddress(), parseEther("2000"), 0, 0, 0, ZeroAddress);
      const clipToken = await Token.deploy("CLIP Model Token", "CLIP", controller.address, await params.getAddress(), parseEther("3000"), 0, 0, 0, ZeroAddress);
    ...
    it("Should handle gas optimization for batch deployments", async function () {
      ...
      for (const config of tokenConfigs) {
        const tx = await Token.deploy(config.name, config.symbol, config.controller, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
  ...
  describe("Cap-based allocation accounting", function () {
    let capToken;

    beforeEach(async function () {
      capToken = await Token.deploy(
        "Cap Token",
        "CAP",
        controller.address,
    ...
    it("enforces the investor allocation independently from rewards", async function () {
      await capToken.connect(controller).mintInvestor(user1.address, parseEther("100"));
      await expect(
        capToken.connect(controller).mintInvestor(user1.address, parseEther("1"))
      ).to.be.revertedWith("Exceeds investor allocation");

      await capToken.connect(controller).mintReward(user1.address, parseEther("1000"));

      expect(await capToken.investorMinted()).to.equal(parseEther("100"));
      expect(await capToken.rewardMinted()).to.equal(parseEther("1000"));
      expect(await capToken.totalSupply()).to.equal(parseEther("1100"));
      expect(await capToken.getRemainingSupply()).to.equal(0);
      expect(await capToken.getRemainingRewardAllocation()).to.equal(
        (await capToken.getRewardMintingCap()) - parseEther("1000")
      );
    });

    it("restores investor headroom when investor-minted tokens are burned", async function () {
      await capToken.connect(controller).mintInvestor(user1.address, parseEther("100"));
      await capToken.connect(controller).burnInvestor(user1.address, parseEther("40"));

      expect(await capToken.investorMinted()).to.equal(parseEther("60"));
      expect(await capToken.getRemainingInvestorAllocation()).to.equal(parseEther("40"));

      await capToken.connect(controller).mintInvestor(user1.address, parseEther("40"));
      expect(await capToken.investorMinted()).to.equal(parseEther("100"));
    });

    it("reverts when burning more investor supply than was minted", async function () {
      await expect(
        capToken.connect(controller).burnInvestor(user1.address, parseEther("1"))
      ).to.be.revertedWith("Burn exceeds investor minted");
    });

    it("uses the live params contract to determine reward minting headroom", async function () {
      expect(await capToken.getRewardMintingCap()).to.equal(wholeTokens(100000));

      await capToken.connect(controller).mintReward(user1.address, wholeTokens(100000));
      await expect(
        capToken.connect(controller).mintReward(user1.address, 1)
      ).to.be.revertedWith("Exceeds reward mint cap");

      await params.connect(governor).setTokensPerDeltaOne(2000);
      expect(await capToken.getRewardMintingCap()).to.equal(wholeTokens(200000));

      await capToken.connect(controller).mintReward(user1.address, wholeTokens(100000));
      expect(await capToken.rewardMinted()).to.equal(wholeTokens(200000));
    });

    it("keeps the legacy mint entrypoint disabled on cap-based tokens", async function () {
      await expect(
        capToken.connect(controller).mint(user1.address, parseEther("1"))
      ).to.be.revertedWith("Use mintInvestor or mintReward on cap-based tokens");
    });
  ...
  describe("Edge Cases and Error Handling", function () {
    it("Should handle very large token names and symbols", async function () {
      ...
      token = await Token.deploy(maxName, maxSymbol, controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    ...
    it("Should handle special characters in token metadata", async function () {
      ...
      token = await Token.deploy(specialName, specialSymbol, controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    ...
    it("Should validate constructor parameters independently", async function () {
      ...
      await expect(Token.deploy("", "VALID", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress))
        .to.be.revertedWith("Token name cannot be empty");

      await expect(Token.deploy("Valid", "", controller.address, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress))
        .to.be.revertedWith("Token symbol cannot be empty");

      await expect(Token.deploy("Valid", "VALID", ZeroAddress, await params.getAddress(), parseEther("1000"), 0, 0, 0, ZeroAddress))
        .to.be.revertedWith("Controller cannot be zero address");
    });
    ...
    it("Should reject zero address params", async function () {
      await expect(Token.deploy("Valid", "VALID", controller.address, ZeroAddress, parseEther("1000"), 0, 0, 0, ZeroAddress))
  ...
  describe("Params Integration", function () {
    ...
    it("Should store params address correctly", async function () {
      expect(await token.params()).to.equal(await params.getAddress());
    });

    it("Should have immutable params address", async function () {
      // Verify params address cannot be changed by checking it's a view function
      const paramsAddress = await token.params();
      expect(paramsAddress).to.equal(await params.getAddress());
    ...
    it("Should access params functions through token", async function () {
      ...
      expect(uri).to.equal("https://test.license");
    });
  });

  describe("Event Filtering and Querying", function () {
    beforeEach(async function () {
      token = await Token.deploy("Test Token", "TEST", controller.address, await params.getAddress(), parseEther("10000"), 0, 0, 0, ZeroAddress);
      await token.waitForDeployment();
    });
