...
| 5M `fuzz-long` (per-harness matrix) | ✅ | Ran 2026-07-01 (run 28535746423). **0 falsified.** 6/14 harnesses completed the full 5M green; the 8 heavier ones ran to the 180-min per-job cap (millions of sequences each) and were cut off by timeout — not by any invariant violation. With the 50k `echidna:all` (all 17, 0 falsified), no property was violated at any depth. CI fixes (#215): pre-compile, per-harness matrix, + the 5 H-6 harnesses the old job omitted. Post-freeze hygiene: lower the scheduled testLimit so the heavy harnesses complete inside the cap. |
...
I confirm all seven freeze gates (§2) are green, the launch parameters (§4) are correct and
intentional, and the risk-accepted items (§6) are acknowledged. `mainnet-rc1` is applied to the
commit that merges this signed sign-off into `main` (the SHA is recorded by the annotated tag;
the reviewed tree is `main` @ `7f3d83b` plus this doc commit — no contract change).

**Approver:** @timogilvie  **Date:** 2026-07-01  **Tag applied:** `mainnet-rc1` ✅
