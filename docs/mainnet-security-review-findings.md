...
| M-6 | Reconcile launch allocations vs lock doc + re-sign; fill all placeholder addresses | @timogilvie | ◐ Placeholders filled (supplier `0xD1Eb…8136`). Allocation drift (HMESS 2.5M/HLEAD 1.25M/HROUT 5M vs lock-doc uniform 1M) — **awaiting @timogilvie's final mainnet numbers**, then reconcile both + extend drift-guard test to allocations + re-sign |
